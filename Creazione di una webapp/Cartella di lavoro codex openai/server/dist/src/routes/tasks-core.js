...
import { clampInt, db, ensureColumn, ensureProject, ensureTask, normalizeTaskOrder, nowIso, parseId, } from '../db.js';
...
tasksCoreRouter.post('/tasks', (req, res) => {
    ...
    const userIds = Array.isArray(req.body?.userIds)
        ? req.body.userIds.map((id) => Number(id)).filter((id) => Number.isInteger(id))
        : [];
    const max = db
    ...
    const result = db
        ...
        .run(projectId, columnId, title, description, priority, color, max.maxPos + 1, estimatedMinutes, actualMinutes * 60, startDate, dueDate, column.name.trim().toUpperCase() === 'DONE' ? now : null, now, now);
    if (userIds.length) {
...
tasksCoreRouter.put('/tasks/:taskId', (req, res) => {
    ...
    db.prepare(`UPDATE tasks
     ...
     WHERE id = ?`).run(title, description, priority, color, estimatedMinutes, actualMinutes * 60, startDate, dueDate, nowIso(), taskId);
...
tasksCoreRouter.patch('/tasks/:taskId/move', (req, res) => {
    ...
    if (!Number.isInteger(toColumnId))
        return res.status(400).json({ error: 'toColumnId obbligatorio' });
    const task = ensureTask(taskId);
...
tasksCoreRouter.patch('/tasks/:taskId/reorder', (req, res) => {
    ...
    const neighbor = db
        ...
        .get(task.column_id, task.position);
    if (!neighbor)
...
tasksCoreRouter.patch('/tasks/:taskId/assignees', (req, res) => {
    ...
    const task = ensureTask(taskId);
    const userIds = Array.isArray(req.body?.userIds)
        ? [...new Set(req.body.userIds.map((id) => Number(id)).filter((n) => Number.isInteger(n) && n > 0))]
        : [];
    const tx = db.transaction(() => {
...
tasksCoreRouter.patch('/tasks/:taskId/time', (req, res) => {
    ...
    const estimatedMinutes = clampInt(req.body?.estimatedMinutes, 0);
    const actualMinutes = clampInt(req.body?.actualMinutes, 0);
    db.prepare('UPDATE tasks SET estimated_minutes = ?, actual_seconds = ?, updated_at = ? WHERE id = ?').run(estimatedMinutes, actualMinutes * 60, nowIso(), taskId);
    db.prepare('UPDATE projects SET updated_at = ? WHERE id = ?').run(nowIso(), task.project_id);
    res.json({ ok: true });
});
tasksCoreRouter.post('/tasks/:taskId/timer/start', (req, res) => {
    ...
    const active = db
        ...
        .get(taskId);
    if (active)
        return res.status(400).json({ error: 'Timer gia attivo su questa task' });
    db.prepare('INSERT INTO timer_entries (task_id, started_at, ended_at, duration_seconds) VALUES (?, ?, NULL, 0)').run(taskId, nowIso());
    db.prepare('UPDATE projects SET updated_at = ? WHERE id = ?').run(nowIso(), task.project_id);
...
tasksCoreRouter.post('/tasks/:taskId/timer/stop', (req, res) => {
    ...
    const active = db
        ...
        .get(taskId);
    if (!active)
        return res.status(400).json({ error: 'Nessun timer attivo da fermare' });
    ...
    const tx = db.transaction(() => {
        ...
        db.prepare('UPDATE tasks SET actual_seconds = actual_seconds + ?, updated_at = ? WHERE id = ?').run(duration, nowIso(), taskId);
    });
    tx();
    const updated = db.prepare('SELECT actual_seconds AS actualSeconds FROM tasks WHERE id = ?').get(taskId);
    db.prepare('UPDATE projects SET updated_at = ? WHERE id = ?').run(nowIso(), task.project_id);
