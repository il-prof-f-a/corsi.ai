...
export function createTasksExtraRouter(uploadSingle, uploadsDir) {
    const router = Router();
    router.get('/tasks/:taskId', (req, res) => {
        ...
        const assignees = db
            .prepare(`SELECT u.id, u.name, u.email, u.avatar_color AS avatarColor
         ...
         ORDER BY u.name ASC`)
            .all(taskId);
        const comments = db
            .prepare(`SELECT c.id, c.task_id AS taskId, c.user_id AS userId, c.content, c.created_at AS createdAt,
        ...
        const attachments = db
            .prepare(`SELECT id, task_id AS taskId, original_name AS originalName, stored_name AS storedName,
                mime_type AS mimeType, size, created_at AS createdAt
         FROM task_attachments
         WHERE task_id = ?
         ORDER BY created_at DESC, id DESC`)
            .all(taskId)
            .map((row) => ({ ...row, url: `/uploads/${row.storedName}` }));
    ...
    router.post('/tasks/:taskId/comments', (req, res) => {
        ...
        if (!content)
            return res.status(400).json({ error: 'Il commento non puo essere vuoto' });
        const result = db
    ...
    router.post('/tasks/:taskId/attachments', uploadSingle, (req, res) => {
        ...
        const result = db
            .prepare(`INSERT INTO task_attachments (task_id, original_name, stored_name, mime_type, size, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
