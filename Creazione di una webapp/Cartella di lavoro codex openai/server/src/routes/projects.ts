...
projectsRouter.post('/projects', (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Il nome del progetto e obbligatorio' });
  ...
  const insertColumn = db.prepare(
    'INSERT INTO columns (project_id, name, position, color, is_default) VALUES (?, ?, ?, ?, 1)',
  );
...
projectsRouter.put('/projects/:projectId', (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Il nome del progetto e obbligatorio' });
  db.prepare('UPDATE projects SET name = ?, description = ?, color = ?, updated_at = ? WHERE id = ?').run(
    name,
    description,
    color,
    nowIso(),
...
projectsRouter.get('/projects/:projectId/board', (req, res) => {
  ...
  if (!board) return res.status(404).json({ error: 'Progetto non trovato' });
  res.json(board);
});
...
projectsRouter.post('/projects/:projectId/columns', (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Il nome della colonna e obbligatorio' });
  const maxPos = db
...
projectsRouter.patch('/projects/:projectId/columns/reorder', (req, res) => {
  ...
  if (!ids.length) return res.status(400).json({ error: 'columnIds obbligatorio' });
  const parsed = ids.map((id: unknown) => Number(id)).filter((id: number) => Number.isInteger(id));
  const upd = db.prepare('UPDATE columns SET position = ? WHERE id = ? AND project_id = ?');
  const tx = db.transaction(() => {
    parsed.forEach((columnId: number, index: number) => upd.run(index, columnId, projectId));
...
projectsRouter.delete('/columns/:columnId', (req, res) => {
  ...
  const column = ensureColumn(columnId);
  const tasks = db.prepare('SELECT id FROM tasks WHERE column_id = ? ORDER BY position ASC').all(columnId) as {
    id: number;
  }[];
  ...
  const tx = db.transaction(() => {
    if (tasks.length > 0 && moveToColumnId) {
      ...
      const max = db
        ...
        .get(moveToColumnId) as { maxPos: number };
