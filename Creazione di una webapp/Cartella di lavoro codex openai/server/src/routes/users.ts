...
usersRouter.post('/users', (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Il nome del collaboratore e obbligatorio' });
...
usersRouter.put('/users/:userId', (req, res) => {
  ...
  if (!current) return res.status(404).json({ error: 'Collaboratore non trovato' });
