...
export function parseId(value: string, name: string) {
  const id = Number(value);
  if (!Number.isInteger(id) || id <= 0) {
    throw Object.assign(new Error(`Parametro '${name}' non valido`), { status: 400 });
  }
  return id;
}
...
export function ensureProject(projectId: number) {
  const row = db.prepare('SELECT * FROM projects WHERE id = ?').get(projectId);
  if (!row) throw Object.assign(new Error('Progetto non trovato'), { status: 404 });
}

export function ensureColumn(columnId: number) {
  const row = db.prepare('SELECT * FROM columns WHERE id = ?').get(columnId) as
    | { id: number; project_id: number; name: string; position: number }
    | undefined;
  if (!row) throw Object.assign(new Error('Colonna non trovata'), { status: 404 });
  return row;
}

export function ensureTask(taskId: number) {
  const row = db.prepare('SELECT * FROM tasks WHERE id = ?').get(taskId) as
    | { id: number; project_id: number; column_id: number; position: number }
    | undefined;
  if (!row) throw Object.assign(new Error('Task non trovata'), { status: 404 });
  return row;
}

export function normalizeColumnOrder(projectId: number) {
  const rows = db
...
export function normalizeTaskOrder(columnId: number) {
  const rows = db
